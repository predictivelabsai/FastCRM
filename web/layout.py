"""FastCRM 3-pane layout: top bar, left nav, center work area, right AI rail.

Server-rendered with FastHTML FT components. All interactivity is HTMX +
a little vanilla JS for the streaming chat (no JS framework).
"""
from __future__ import annotations

from fasthtml.common import (
    Div, H1, H3, H4, P, Span, A, Button, Form, Input, Title, Link,
    Script, Style, NotStr,
)

# FastCRM palette — indigo primary, emerald "won", rose "lost".
LAYOUT_CSS = """
:root{
  --bg:#f5f6fb; --surface:#ffffff; --surface-2:#eef0f8; --border:#e1e4f0;
  --text:#1b2236; --text-dim:#525a72; --text-mute:#8b93ac;
  --accent:#4f46e5; --accent-hover:#4338ca; --accent-light:#e7e6fb;
  --won:#16a34a; --won-light:#dcfce7; --lost:#e11d48; --lost-light:#ffe4e6;
  --warn:#d97706;
}
...
.app{display:grid;grid-template-columns:230px 1fr var(--rail,340px);
  grid-template-rows:52px 1fr;grid-template-areas:"top top top" "left center right";
...
#copilot-reopen{position:fixed;right:0;bottom:26px;display:none;align-items:center;gap:6px;
  cursor:pointer;z-index:60;background:var(--accent);color:#fff;font-size:13px;font-weight:600;
...
.copilot-min,.copilot-exp{cursor:pointer;border:1px solid var(--border);background:var(--surface);
...
.brand-dot{width:11px;height:11px;background:var(--accent);border-radius:3px;transform:rotate(45deg);display:inline-block;}
.env-pill{background:var(--accent-light);color:var(--accent-hover);padding:3px 10px;border-radius:999px;
...
.nav-section h4{margin:6px 16px 4px;font-size:11px;text-transform:uppercase;letter-spacing:.8px;
  color:var(--text-mute);font-weight:700;}
.nav-item{display:flex;align-items:center;gap:9px;padding:8px 16px;color:var(--text-dim);
...
.kpi .value{font-size:25px;font-weight:700;margin-top:4px;color:var(--text);}
.kpi .trend{font-size:12px;color:var(--text-mute);margin-top:2px;}
.kpi::after{content:'';position:absolute;top:0;right:0;bottom:0;width:4px;background:var(--accent);}
.kpi.won::after{background:var(--won);} .kpi.warn::after{background:var(--warn);}
...
.pill.contacted{background:#e0e7ff;color:#4338ca;}
.pill.nurture{background:#fef3c7;color:#92400e;}
.pill.qualified{background:var(--won-light);color:#166534;}
.pill.unqualified{background:#f1f5f9;color:#475569;}
.pill.junk{background:var(--lost-light);color:#9f1239;}
.pill.won{background:var(--won-light);color:#166534;}
.pill.lost{background:var(--lost-light);color:#9f1239;}
.pill.high{background:var(--lost-light);color:#9f1239;}
.pill.medium{background:#fef3c7;color:#92400e;}
.pill.low{background:#f1f5f9;color:#475569;}
.pill.done{background:var(--won-light);color:#166534;}
.pill.todo,.pill.backlog{background:#f1f5f9;color:#475569;}
.pill.in.progress,.pill.inprogress{background:#dbeafe;color:#1d4ed8;}

/* kanban */
.kanban{display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;align-items:flex-start;}
.kan-col{flex:0 0 252px;background:var(--surface-2);border:1px solid var(--border);border-radius:10px;
  display:flex;flex-direction:column;max-height:calc(100vh - 220px);}
.kan-head{padding:10px 12px;font-weight:700;font-size:13px;display:flex;justify-content:space-between;
  align-items:center;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--surface-2);border-radius:10px 10px 0 0;}
.kan-head .cnt{font-size:11px;color:var(--text-mute);font-weight:600;}
.kan-head .stage-dot{width:8px;height:8px;border-radius:50%;display:inline-block;margin-right:6px;}
.kan-body{padding:8px;overflow-y:auto;display:flex;flex-direction:column;gap:8px;}
.kan-card{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:10px 12px;
  cursor:pointer;display:block;color:var(--text);}
.kan-card:hover{border-color:var(--accent);text-decoration:none;box-shadow:0 2px 8px rgba(79,70,229,.1);}
.kan-card .org{font-weight:600;font-size:13px;}
.kan-card .val{color:var(--accent-hover);font-weight:700;font-size:14px;margin-top:3px;}
.kan-card .meta{color:var(--text-mute);font-size:11.5px;margin-top:4px;display:flex;justify-content:space-between;}

/* funnel bars */
.funnel-row{display:grid;grid-template-columns:140px 1fr 90px;align-items:center;gap:10px;margin-bottom:7px;font-size:13px;}
.funnel-bar{height:18px;border-radius:5px;background:var(--accent);min-width:2px;}
.funnel-row .v{text-align:right;color:var(--text-dim);font-variant-numeric:tabular-nums;}
...
.timeline li{padding:10px 0 10px 22px;border-left:2px solid var(--border);position:relative;margin-left:4px;}
.timeline li::before{content:'';position:absolute;left:-6px;top:13px;width:10px;height:10px;border-radius:50%;
  background:var(--accent);border:2px solid var(--surface);}
.timeline .when{color:var(--text-mute);font-size:11.5px;}
.timeline .kind{font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--accent-hover);font-weight:600;}
...
.login-wrap{height:100vh;display:flex;align-items:center;justify-content:center;
  background:linear-gradient(135deg,#eef0fb 0%,#e7e6fb 100%);}
...
.login-card .error{color:var(--lost);font-size:12px;margin:6px 0;}
...
.right-header{padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;}
...
.msg pre{background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:8px;font-size:12px;white-space:pre-wrap;}
...
.msg th{background:var(--text);color:#fff;font-size:10.5px;text-transform:uppercase;}
.msg th,.msg td{text-align:left;padding:5px 7px;border:1px solid var(--border);overflow-wrap:anywhere;}
.msg tbody tr:nth-child(even) td{background:var(--surface-2);}
.chat-input{border-top:1px solid var(--border);padding:10px;background:var(--surface);}
...
.chat-send-btn{display:inline-flex;align-items:center;background:var(--accent);color:#fff;border:none;border-radius:8px;
...
.sample-card{display:flex;align-items:center;gap:8px;background:var(--bg);border:1px solid var(--border);padding:9px 12px;
  border-radius:10px;font-size:12.5px;cursor:pointer;color:var(--text-dim);width:100%;text-align:left;line-height:1.35;margin-bottom:6px;font-family:inherit;}
.sample-card::before{content:"💬";flex-shrink:0;}
...
.callout{background:var(--accent-light);border:1px solid var(--border);border-left:4px solid var(--accent);
  color:var(--accent-hover);padding:12px 16px;border-radius:8px;margin-bottom:18px;font-size:13px;line-height:1.55;}
"""

STAGE_COLORS = {
    "Qualification": "#6366f1", "Demo/Making": "#8b5cf6", "Proposal/Quotation": "#0ea5e9",
    "Negotiation": "#f59e0b", "Ready to Close": "#14b8a6", "Won": "#16a34a", "Lost": "#e11d48",
}
...
def topbar(env: str, user_email: str | None):
    right = Div(
        Button(NotStr("&laquo; Chat"), id="copilot-topbar-toggle", cls="btn",
    ...
    return Div(
        Div(Span(cls="brand-dot"),
            ...
            Span("CRM", style="color:var(--accent);font-weight:700;letter-spacing:1px;"),
            cls="brand"),
        right, cls="topbar",
    )
...
def _sample_cards():
    cards = [Button(Span(q), cls="sample-card", onclick=f"fillChat({q!r});sendMessage(null);", title=q)
             for q in SAMPLE_QUESTIONS]
    return Div(Div(Span("Try asking:", cls="sample-cards-label")),
               Div(*cards), cls="sample-cards")


def right_pane_chat(thread_id: str):
    return Div(
        Div(H3("AI Assistant"),
            Div(Button("New", cls="btn", hx_get="/chat/new", hx_target="#chat-body",
                       hx_swap="innerHTML", title="Start new thread"),
                Button(NotStr("&laquo;"), id="copilot-exp-btn", cls="copilot-exp",
                       onclick="toggleExpand()", title="Expand / shrink"),
                Button(NotStr("&rsaquo;"), cls="copilot-min", onclick="toggleCopilot()",
                       title="Minimise"),
                cls="tabs"),
            cls="right-header"),
        Div(
            Div(P("Ask about your pipeline, leads, deals or tasks — or tap a question below.",
            ...
            Form(
                ...
                Div(Input(type="text", name="message", id="chat-input",
                          placeholder="Ask a question or /pipeline /leads /help …", autocomplete="off"),
...
def page(active: str, env: str, user_email: str, thread_id: str, *content, right_override=None):
    ...
    return (
        ...
        Style(LAYOUT_CSS),
        Div(topbar(env, user_email), left_pane(active),
            Div(*content, cls="center-pane"), right,
            Div(NotStr("&lsaquo; AI Assistant"), id="copilot-reopen",
                onclick="toggleCopilot()", title="Show assistant"),
            cls="app"),
        Script(LAYOUT_JS),
    )


def kpi_card(label, value, trend="", won=False, warn=False):
    cls = "kpi" + (" won" if won else "") + (" warn" if warn else "")
    ...
    return Div(Div(label, cls="label"), Div(val, cls="value"),
               Div(trend, cls="trend") if trend else None, cls=cls)
...
function toggleCopilot(){var app=document.querySelector('.app');if(!app)return;
  app.classList.toggle('right-collapsed');if(app.classList.contains('right-collapsed'))app.classList.remove('right-expanded');
  try{localStorage.setItem('crmCollapsed',app.classList.contains('right-collapsed')?'1':'0');}catch(e){}_sync();}
function toggleExpand(){var app=document.querySelector('.app');if(!app)return;
  app.classList.remove('right-collapsed');app.classList.toggle('right-expanded');
  try{localStorage.setItem('crmExpanded',app.classList.contains('right-expanded')?'1':'0');localStorage.setItem('crmCollapsed','0');}catch(e){}_sync();}
(function(){try{var app=document.querySelector('.app');if(!app)return;
  if(localStorage.getItem('crmCollapsed')==='1')app.classList.add('right-collapsed');
  else if(localStorage.getItem('crmExpanded')==='1')app.classList.add('right-expanded');}catch(e){}})();
...
function showThinking(){var cb=document.getElementById('chat-body');if(!cb)return;
  _thinker={t:Date.now(),el:document.createElement('div')};_thinker.el.className='thinking-indicator';
